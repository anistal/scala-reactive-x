...
object Async {
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] =
    eventuallyX.map(_.intValue()).recoverWith {
      case ex: Exception if NonFatal(ex) => Future.successful(-1)
    }
  ...
  def sequenceComputations[A, B](
    ...
    for {
      resultComputation1 <- makeAsyncComputation1()
      resultComputation2 <- makeAsyncComputation2()
    } yield { (resultComputation1, resultComputation2) }
  ...
  def concurrentComputations[A, B](
    ...
    val computation1: Future[A] = makeAsyncComputation1()
    val computation2 = makeAsyncComputation2()

    for {
      resultComputation1 <- computation1
      resultComputation2 <- computation2
    } yield { (resultComputation1, resultComputation2) }
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
      makeAsyncComputation().map(x => x).recoverWith {
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    ...
    callbackBasedApi.computeIntAsync(x => x match {
